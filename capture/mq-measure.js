const { chromium } = require('playwright');
const http=require('http'),fs=require('fs'),path=require('path');
const OUT=path.join(__dirname,'out'),PORT=8082;
const M={'.html':'text/html','.css':'text/css','.js':'text/javascript','.avif':'image/avif','.png':'image/png','.jpeg':'image/jpeg','.svg':'image/svg+xml','.webm':'video/webm','.mp4':'video/mp4','.woff2':'font/woff2'};
const srv=http.createServer((q,r)=>{const rel=decodeURIComponent(q.url.split('?')[0]).replace(/^\/+/,'')||'index.html';const f=path.join(OUT,rel);
 if(!f.startsWith(OUT)||!fs.existsSync(f)||fs.statSync(f).isDirectory()){r.writeHead(404);return r.end();}
 const b=fs.readFileSync(f),t=M[path.extname(f).toLowerCase()]||'application/octet-stream';
 r.writeHead(200,{'Content-Type':t,'Content-Length':b.length});r.end(b);});
(async()=>{
 await new Promise(r=>srv.listen(PORT,r));
 const br=await chromium.launch({channel:'chrome'});
 const pg=await br.newPage({viewport:{width:1440,height:900},deviceScaleFactor:2});
 await pg.goto(`http://localhost:${PORT}/index.html`,{waitUntil:'load',timeout:60000});
 await pg.waitForTimeout(7000);
 await pg.evaluate(()=>window.scrollTo({top:1500,behavior:'instant'}));
 await pg.waitForTimeout(3000);
 const d=await pg.evaluate(()=>{
   const out=[];
   document.querySelectorAll('.nav-comapny-item')[0].children.length;
   [...document.querySelectorAll('.nav-comapny-item')[0].children].forEach((el,i)=>{
     const r=el.getBoundingClientRect();
     const img=el.querySelector('image')||el.querySelector('img');
     const href=img?(img.getAttribute('href')||img.getAttribute('src')||''):'';
     const vb=el.getAttribute?el.getAttribute('viewBox'):null;
     out.push({i,tag:el.tagName,cls:(el.className.baseVal!==undefined?el.className.baseVal:el.className)||'',
               w:+r.width.toFixed(1),h:+r.height.toFixed(1),vb,logo:href.split('/').pop()});
   });
   return out;
 });
 console.log('slot   rendered      viewBox        logo');
 d.forEach(s=>console.log(`[${s.i}] ${String(s.w).padStart(6)}x${String(s.h).padStart(5)}  ${String(s.vb||'-').padEnd(13)} ${s.logo}  (.${s.cls.split(' ')[0]})`));
 const el=await pg.$('.nav-comapny-wrap');
 if(el) await el.screenshot({path:path.join(OUT,'shots','marquee.png'),animations:'disabled',timeout:60000});
 await br.close(); srv.close();
})();
