...
const OUT=path.join(__dirname,'out'),PORT=8088;
...
(async()=>{
 ...
 pg.on('requestfailed',r=>{const e=r.failure()?.errorText||'';if(!e.includes('ERR_ABORTED'))bad.push(r.url().slice(-40));});
 ...
 const cards=await pg.$$('.about-card');
 for (const idx of [0,3,4,6]) {
   if(!cards[idx]) continue;
   await cards[idx].scrollIntoViewIfNeeded().catch(()=>{});
   await pg.waitForTimeout(2200);
   await cards[idx].screenshot({path:path.join(OUT,'shots',`card-${idx}.png`),animations:'disabled',timeout:60000}).catch(e=>console.log('shot fail',idx,e.message.slice(0,40)));
 }
